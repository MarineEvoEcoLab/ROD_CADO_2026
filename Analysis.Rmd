...
flank         <- 20000        # bp either side of the focal SNP
binsize       <- 500          # bp, for the focal-region diversity tracks
smooth.bins   <- 10            # centred rolling mean over this many bins; 1 = raw
show.pca      <- FALSE        # TRUE puts the outlier PCA back in as a panel
ins.top       <- "LFMM"       # top row of the focal panels: "LFMM" or "FST"
strip.theme   <- NA           # show the GO theme on the panel strip; NA = only if <= 6 panels
...
# the theme makes the strip too wide once there are many columns
show.theme <- if (is.na(strip.theme)) nrow(focal) <= 6 else isTRUE(strip.theme)
focal$sub <- paste0("Chr", focal$CHR,
                    if (show.theme)
                      ifelse(is.na(focal$theme) | !nzchar(focal$theme), "",
                             paste0(" \u00b7 ", focal$theme))
                    else "")
focal$region <- tolower(as.character(as.roman(seq_len(nrow(focal)))))
focal$lab    <- paste0(focal$region, ". ", focal$name, "\n", focal$sub)
...
fig2.ins <- ggplot(fig2.ins.df, aes(x = rel, y = value, colour = Treatment)) +
  ...
  geom_point(data = top.rows, inherit.aes = FALSE, aes(rel, value),
             colour = "grey78", size = 0.2, alpha = 0.35, shape = 16) +

...
focal <- data.frame(
  ...
  name   = c("semaphorin rec.", "Cul2-RING Ub ligase", "PP2A reg. sub.", "HDAC6"),
...
## Panel A — genome-wide FST

```{r}
focal$BPcum <- vapply(seq_len(nrow(focal)), function(i) {
  sub <- don[don$CHROM == focal$CHROM[i], ]
  sub$BPcum[which.min(abs(sub$BIN_END - focal$POS[i]))]
}, numeric(1))

fst.ymax <- max(don$WEIGHTED_FST, na.rm = TRUE) * 1.20

fig2.man <- ggplot(don, aes(x = BPcum, y = WEIGHTED_FST)) +
  geom_point(aes(color = as.factor(CHR)), alpha = 0.25, size = 0.4) +
  geom_point(data = subset(don, OUTLIER == TRUE), aes(color = as.factor(CHR)),
             shape = 17, alpha = 1, size = 1.6) +
  geom_segment(data = focal, aes(x = BPcum, xend = BPcum,
                                 y = fst.ymax * 0.80, yend = fst.ymax * 0.90),
               inherit.aes = FALSE, linewidth = 0.4) +
  geom_text(data = focal, aes(x = BPcum, y = fst.ymax * 0.97, label = region),
            inherit.aes = FALSE, size = 3, fontface = "bold") +
  scale_color_manual(values = rep(c("#7d5577", "#52374e"), 5)) +
  scale_x_continuous(label = axisdf$CHR, breaks = axisdf$center, expand = c(0.005, 0)) +
  scale_y_continuous(expand = c(0, 0),
                     limits = c(min(don$WEIGHTED_FST, na.rm = TRUE) - 0.005, fst.ymax)) +
  guides(colour = "none", alpha = "none", fill = "none") +
  labs(x = "Chromosome", y = "*F<sub>ST</sub>*") +
...
fig2.ins <- ggplot(fig2.ins.df, aes(x = rel, y = value, colour = Treatment)) +
  ...
  theme(strip.background = element_blank(),
        strip.text.x = element_text(size = 7, face = "bold", lineheight = 1.1),
...
fig2 <- fig2.man / (fig2.pca | fig2.ins) +
  plot_layout(heights = c(1, 2.6), widths = c(0.8, 1.2), guides = "collect") +

...
```
# Figure 2 — Genome-wide differentiation, outlier PCA, and focal outlier regions

Assembles a full-page figure: (A) genome-wide *F*~ST~ Manhattan with OutFLANK outliers
flagged, (B) PCA of the union outlier set (LFMM2 ∪ OutFLANK), and (C) a grid of focal
outlier regions showing per-SNP *F*~ST~, π, observed heterozygosity, and Tajima's *D*
by treatment.

Because LD decays over hundreds of bp to a few kb in *C. virginica*, panel C uses a ±20 kb
window and re-bins the diversity metrics at 500 bp within the focal regions only. The
genome-wide 10 kb bins used elsewhere in this document are far too coarse at this scale —
they give three points per window.

Depends on objects built above: `don` / `axisdf` (Manhattan), `pca_data_combined`,
`pve_combined`, `p_value_combined` (PCA), and `mydf` (per-SNP *F*~ST~).

## Candidate outlier table

Union outlier set with FST, the method(s) that flagged it, and the overlapping gene.
Use this to choose the focal regions plotted in panel C.
...
outlier.candidates <- unique(rbind(outliers.lfmm, outliers.of)[, .(CHROM, POS)])[
  order(CHROM, POS)]
outlier.candidates[, `:=`(
  CHR  = chr.map[CHROM],
  LFMM = paste(CHROM, POS) %in% paste(outliers.lfmm$CHROM, outliers.lfmm$POS),
  OF   = paste(CHROM, POS) %in% paste(outliers.of$CHROM,   outliers.of$POS))]

# gene the SNP sits inside (0-based start, so compare against POS - 1)
outlier.candidates[, GENE := mapply(function(cc, pp) {
  g <- genes.bed[CHROM == cc & GSTART <= pp - 1 & GEND > pp - 1, GENE]
  paste(unique(g), collapse = "; ")
}, CHROM, POS)]

# per-SNP FST, joined from the Manhattan table
fst.snp <- as.data.table(mydf)[, .(CHROM, POS = BIN_END, FST = WEIGHTED_FST)]
setkey(fst.snp, CHROM, POS)
outlier.candidates <- fst.snp[outlier.candidates, on = .(CHROM, POS)]

outlier.candidates[order(-FST)][1:25]
```

`FST` is `NA` for LFMM-only outliers that fall below the maf 0.05 threshold used for the
OutFLANK set — those SNPs are absent from the *F*~ST~ Manhattan by construction.

## Focal regions for panel C

**Edit this table to change which loci appear as insets.** One row per region; `region` is
the label drawn on panel A, `name` is the short gene label on the inset strip. Re-run every
chunk from here down after editing — the 500 bp re-binning is derived from this table.

```{r}
flank   <- 20000  # bp either side of the focal SNP; oyster LD decays well within this
binsize <- 2500    # bp, for the focal-region diversity tracks (used by the bash chunk below)

# Display smoothing for the panel C diversity tracks: centred rolling mean over this many
# bins. 1 = raw. At binsize 500 this makes 3 = 1.5 kb, 5 = 2.5 kb, 9 = 4.5 kb.
# Change this and re-run panel C only — no need to touch the bash chunk.
smooth.bins <- 5

focal <- data.frame(
  region = c("i", "ii", "iii", "iv"),
  CHROM  = c("NC_035787.1", "NC_035781.1", "NC_035783.1", "NC_035787.1"),
  POS    = c(60756955,      22817136,      12033923,      43870981),
  gene   = c("LOC111108439","LOC111118776","LOC111129890","LOC111107402"),
  name   = c("semaphorin", "Cul2-RING Ub", "PP2A reg. sub.", "HDAC6"),
...
## Re-bin diversity metrics at 500 bp within the focal regions

Same tools and options as the genome-wide sections above, restricted to the focal regions
and binned at `binsize` instead of 10 kb. Cheap — seconds, not hours.
...
# --- per-SNP FST in the focal regions -----------------------------------
...
fst.focal <- fread("./analysis/figure2/focal_fst_persnp.txt", header = TRUE)
...
Uses the LFMM ridge p-values rather than *F*~ST~. The p-value vector is in the same SNP
order as `totalloci` (both derive from the biallelic subset of `SNP.TRSdp10g1.FIL.vcf.gz`),
which is the mapping the outlier-index chunks above already rely on.

Set `lfmm.var` to choose which environmental variable is shown. `Combined` is the sequential
CADO + ROD treatment, which carries 107 of the 115 LFMM outliers and is the comparison the
Discussion is built around.

```{r}
lfmm.var <- "Combined"     # "Combined" | "CADO" | "Disease"
...
stopifnot(nrow(lfmm.loci) == length(lfmm.pv),
          length(lfmm.pv) == length(lfmm.qv))
...
# cumulative x-position, same rank-based scheme as the FST Manhattan above
...
focal$BPcum <- vapply(seq_len(nrow(focal)), function(i) {
  ...
  sub$BPcum[which.min(abs(sub$POS - focal$POS[i]))]
}, numeric(1))
...
## Panel B — PCA of union outlier loci
...
fig2.pca <- ggplot(pca_data_combined, aes(x = PC1, y = PC2, color = Treatment)) +
  ...
  guides(colour = "none") +   # panel C supplies the single collected legend
  theme_classic(base_size = 10)

fig2.pca
```

## Panel C — focal outlier regions

The *F*~ST~ row plots every SNP in the window as a grey point, with a 51-SNP centred rolling
mean as the line (~1.3 kb at this SNP density) and the focal outlier as a red triangle.
The lower three rows are the 500 bp re-binned tracks.

```{r}
# --- per-SNP FST + rolling mean ---------------------------------------------
fst.rows <- rbindlist(lapply(seq_len(nrow(focal)), function(i) {
  f <- focal[i, ]
  s <- fst.focal[CHROM == f$CHROM & abs(POS - f$POS) <= flank][order(POS)]
  if (!nrow(s)) return(NULL)
  data.table(region = f$lab, rel = (s$POS - f$POS) / 1000, value = s$FST,
             roll = frollmean(s$FST, 51, align = "center"),
             is.focal = s$POS == f$POS)
}))

# --- 500 bp windowed metrics -------------------------------------------------
...
metric.lv <- c("italic(F)[ST]", "pi", "H[o]", "Tajima*minute*s~italic(D)")
...
fst.rows[, `:=`(metric = factor("italic(F)[ST]", levels = metric.lv),
                region = factor(region, levels = focal$lab))]
...
fig2.ins <- ggplot(fig2.ins.df, aes(x = rel, y = value, colour = Treatment)) +
  ...
  geom_point(data = fst.rows, inherit.aes = FALSE, aes(rel, value),
             colour = "grey78", size = 0.2, alpha = 0.35) +
  geom_line(data = fst.rows, inherit.aes = FALSE, aes(rel, roll),
            colour = "grey20", linewidth = 0.5, na.rm = TRUE) +
  geom_point(data = fst.rows[is.focal == TRUE], inherit.aes = FALSE, aes(rel, value),
...
```{r}
fig2 <- fig2.man / (fig2.pca | fig2.ins) +
...
png(filename = "figures/Figure2.png", type = "cairo", units = "in",
    width = 7.5, height = 9, res = 300, bg = "transparent")
print(fig2)
null <- dev.off()
```

...
n.focal       <- 6            # number of focal regions to inset
...
if (focal.by == "file") {
  ...
  fl <- fread(f.path)
  stopifnot(all(c("CHROM", "POS") %in% names(fl)))
  focal <- as.data.frame(head(fl, n.focal))
  focal$CHR  <- chr.map[focal$CHROM]
  focal$gene <- mapply(gene.at, focal$CHROM, focal$POS)
  if (!"name" %in% names(focal))
    focal$name <- ifelse(focal$gene %in% names(gene.labels), gene.labels[focal$gene],
                  ifelse(nzchar(focal$gene), focal$gene, "intergenic"))
  focal$sub <- paste0("Chr", focal$CHR,
                      if ("theme" %in% names(focal))
                        ifelse(is.na(focal$theme), "", paste0(" \u00b7 ", focal$theme)) else "")
  # position on the Manhattan
  ...
  cat("focal loci read from", f.path, "\n")
  ...
  focal <- as.data.frame(cand2[keep])
  focal$gene <- mapply(gene.at, focal$CHROM, focal$POS)
  focal$name <- ifelse(focal$gene %in% names(gene.labels), gene.labels[focal$gene],
                ifelse(nzchar(focal$gene), focal$gene, "intergenic"))
  focal$sub  <- paste0("Chr", focal$CHR)
...
if (nrow(focal) < n.focal)
  warning("only ", nrow(focal), " loci met the selection criteria")

focal$region <- tolower(as.character(as.roman(seq_len(nrow(focal)))))
focal$lab    <- paste0(focal$region, ". ", focal$name, "\n", focal$sub)

focal[, intersect(c("region","CHROM","POS","CHR","name","theme","sep_score","P"), names(focal))]
```
...
lfmm.ymax <- max(lfmm.man$LOGP, na.rm = TRUE) * 1.20
...
fig2.man <- ggplot(man.plot, aes(x = BPcum, y = LOGP)) +
  ...
  geom_segment(data = focal, aes(x = BPcum, xend = BPcum,
                                 y = lfmm.ymax * 0.80, yend = lfmm.ymax * 0.90),
               inherit.aes = FALSE, linewidth = 0.4) +
  geom_text(data = focal, aes(x = BPcum, y = lfmm.ymax * 0.97, label = region),

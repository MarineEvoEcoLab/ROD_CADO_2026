...
Assembles a full-page figure: (A) genome-wide LFMM Manhattan with FDR outliers flagged,
and (B) a grid of focal outlier regions showing the per-SNP association signal, π, observed
heterozygosity, and Tajima's *D* by treatment. The outlier PCA is optional — set
`show.pca <- TRUE` to include it as a third panel.

Because LD decays over hundreds of bp to a few kb in *C. virginica*, the focal panels use a
±20 kb window and re-bin the diversity metrics at 500 bp within those regions only. The
genome-wide 10 kb bins used elsewhere are far too coarse at this scale.

Depends on objects built above: the LFMM fits (`oys_rod_CB.pv`, `adj.pv_rod_CADO`,
`adj.pv_rod_DIS` and their q-values), `mydf` (per-SNP *F*~ST~), and — only when
`show.pca = TRUE` — `pca_data_combined`, `pve_combined`, `p_value_combined`.

## Settings

Everything tunable in one place. Changing `flank`, `binsize`, `n.focal` or `lfmm.var` means
re-running the bash chunk below; changing `smooth.bins`, `show.pca` or `ins.top` does not.
...
genic.only  <- TRUE       # TRUE skips SNPs with no overlapping gene when picking focal loci
flank       <- 20000       # bp either side of the focal SNP
binsize     <- 2500         # bp, for the focal-region diversity tracks
...
# against NCBI — extend this vector as products are confirmed. Anything absent falls back
# to the LOC id.
gene.labels <- c(
...
already rely on. `Combined` is the sequential CADO + ROD treatment and carries 107 of the
115 LFMM outliers.
...
Taken straight off the top of the Manhattan: the `n.focal` most significant SNPs, greedily
thinned so no two are within `min.sep` of each other. This is what keeps the inset columns
consistent with the peaks marked in panel A — hand-picking them by *F*~ST~ is what made the
marks land off the peaks previously.

To override, replace `focal` with your own data.frame carrying `CHROM`, `POS` and `name`,
then run the rest of the chunk.

```{r}
cand <- lfmm.man[order(P)]
keep <- integer(0)
for (i in seq_len(nrow(cand))) {
  if (length(keep) >= n.focal) break
  if (length(keep) &&
      any(cand$CHROM[keep] == cand$CHROM[i] & abs(cand$POS[keep] - cand$POS[i]) < min.sep))
    next
  if (genic.only && !nzchar(gene.at(cand$CHROM[i], cand$POS[i]))) next
  keep <- c(keep, i)
}
if (length(keep) < n.focal)
  warning("only ", length(keep), " loci met the selection criteria")

focal <- as.data.frame(cand[keep, .(CHROM, POS, CHR, P, Q, LOGP, BPcum)])
focal$gene   <- mapply(gene.at, focal$CHROM, focal$POS)
focal$name   <- ifelse(focal$gene %in% names(gene.labels),
                       gene.labels[focal$gene],
...
focal[, c("region", "CHROM", "POS", "CHR", "P", "gene", "name")]
```

```{r}
dir.create("./analysis/figure2", showWarnings = FALSE)
withr::with_options(c(scipen = 10),
  write.table(data.frame(focal$CHROM, focal$POS - flank - 1, focal$POS + flank),
              "./analysis/figure2/focal_regions.bed",
              sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE))

# the bash chunk reads the bin size from here, so it can never drift from `binsize`
write(binsize, "./analysis/figure2/binsize.txt")
```

## Re-bin diversity metrics within the focal regions

Same tools and options as the genome-wide sections above, restricted to the focal regions
and binned at `binsize` instead of 10 kb. Cheap — seconds, not hours. **Re-run this whenever
`focal`, `flank` or `binsize` changes.**
...
sort -k1,1 -k2,2n focal_regions.bed > focal_regions.sorted.bed

# --- per-SNP FST in the focal regions (only needed when ins.top = "FST") --
bedtools intersect -a <(sort -k1,1 -k2,2n ../FST/FST_Outflank.bed) \
                   -b focal_regions.sorted.bed -wa -sorted \
  | mawk 'BEGIN{print "CHROM\tPOS\tFST\tOUTLIER"} {print $1 "\t" $3 "\t" $4 "\t" $5}' \
  > focal_fst_persnp.txt

# --- nucleotide diversity ------------------------------------------------
for T in con-con str-con con-rod str-rod; do
  bcftools view --threads 20 -R focal_regions.sorted.bed $VCF \
...
bedtools makewindows -b focal_regions.sorted.bed -w $BIN > focal_bins.bed
...
```{r}
# p-value corresponding to the q < 0.1 cutoff actually used for outlier calling
fdr.line  <- if (any(lfmm.man$Q < 0.1)) -log10(max(lfmm.man[Q < 0.1, P])) else NA_real_
lfmm.ymax <- max(lfmm.man$LOGP, na.rm = TRUE) * 1.20

fig2.man <- ggplot(lfmm.man, aes(x = BPcum, y = LOGP)) +
  geom_point(aes(color = as.factor(CHR)), alpha = 0.25, size = 0.4) +
  ...
  scale_x_continuous(label = lfmm.axis$CHR, breaks = lfmm.axis$center,
                     expand = c(0.005, 0)) +
  scale_y_continuous(expand = c(0, 0), limits = c(0, lfmm.ymax)) +
  guides(colour = "none", alpha = "none", fill = "none") +
...
Note the ascertainment issue: the union outlier set includes 107 SNPs that LFMM selected on
...
if (show.pca) {
  fig2.pca <- ggplot(pca_data_combined, aes(x = PC1, y = PC2, color = Treatment)) +
    ...
    guides(colour = "none") +   # the focal panel supplies the single collected legend
    theme_classic(base_size = 10)
}
...
Top row is the per-SNP association signal (or *F*~ST~ if `ins.top = "FST"`) as grey points
with a 51-SNP centred rolling mean and the focal SNP in red. The lower three rows are the
`binsize` bp tracks, smoothed over `smooth.bins` bins for display.
...
top.src <- if (ins.top == "LFMM") {
  ...
  as.data.table(fread("./analysis/figure2/focal_fst_persnp.txt",
                      header = TRUE))[, .(CHROM, POS, VAL = FST)]
}
...
top.rows <- rbindlist(lapply(seq_len(nrow(focal)), function(i) {
  ...
  data.table(region = f$lab, rel = (s$POS - f$POS) / 1000, value = s$VAL,
             roll = frollmean(s$VAL, 51, align = "center"),
             is.focal = s$POS == f$POS)
}))

# Upstream tools spell these differently (VCF-kit writes TajimaD, not TAJD). Resolve the
# wanted column by name first, then by pattern, so this chunk runs standalone against
# whatever the read chunk above produced.
resolve.col <- function(d, valcol) {
  ...
  message("resolve.col: using '", hit[1], "' for '", valcol, "'")
  hit[1]
}
...
fig2.ins <- ggplot(fig2.ins.df, aes(x = rel, y = value, colour = Treatment)) +
  ...
  geom_point(data = top.rows, inherit.aes = FALSE, aes(rel, value),
             colour = "grey78", size = 0.2, alpha = 0.35) +
...
png(filename = "figures/Figure2.png", type = "cairo", units = "in",
    width = fig.w, height = fig.h, res = 300, bg = "transparent")
print(fig2)
null <- dev.off()
```
